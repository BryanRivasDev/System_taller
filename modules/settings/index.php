...
$defined_modules = [
    ...
    'schedule_manage' => 'Administrar Eventos de Agenda',
    'schedule_view_all' => 'Ver Agenda de Todos los Técnicos',
    'tech_agenda' => 'Mi Agenda Técnica (Vista Técnico)',
    'master_visit_control' => 'Control Maestro de Visitas (Agenda)',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_general_settings') {
        if (!can_access_module('settings_general', $pdo) && !can_access_module('settings', $pdo))
            die("Acceso denegado.");
        $active_tab = 'general';
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'create_role') {
        if (!can_access_module('settings_roles', $pdo) && !can_access_module('settings', $pdo))
            die("Acceso denegado.");
        $active_tab = 'roles';
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_permissions') {
        if (!can_access_module('settings_modules', $pdo) && !can_access_module('settings', $pdo))
            die("Acceso denegado.");
        $active_tab = 'modules';
        try {
...
<div class="animate-enter">
    ...
    <!-- Tabs Header -->
    <div
        style="display: flex; gap: 1rem; margin-bottom: 1.5rem; border-bottom: 1px solid var(--border-color); overflow-x: auto;">
        <button onclick="switchTab('general')" id="tab-btn-general"
            class="tab-btn <?php echo $active_tab == 'general' ? 'active' : ''; ?>">
            <i class="ph ph-gear"></i> General
        </button>
        <button onclick="switchTab('roles')" id="tab-btn-roles"
            class="tab-btn <?php echo $active_tab == 'roles' ? 'active' : ''; ?>">
            <i class="ph ph-identification-badge"></i> Roles
        </button>
        <button onclick="switchTab('modules')" id="tab-btn-modules"
            class="tab-btn <?php echo $active_tab == 'modules' ? 'active' : ''; ?>">
            <i class="ph ph-squares-four"></i> Control de Módulos
        </button>
        <button onclick="switchTab('users')" id="tab-btn-users"
            class="tab-btn <?php echo $active_tab == 'users' ? 'active' : ''; ?>">
            <i class="ph ph-users"></i> Usuarios
        </button>
        <button onclick="switchTab('restore')" id="tab-btn-restore"
            class="tab-btn <?php echo $active_tab == 'restore' ? 'active' : ''; ?>">
    ...
    <?php else: ?>
            ...
            <?php if ($subtab === 'roles'): ?>
                    ...
                    $modules_config = [
                        ...
                        'audit_logs' => ['label' => 'Auditoría', 'cat' => 'Administración', 'icon' => 'ph-shield-checkered'],
                        ...
                        'categories' => ['label' => 'Categorías', 'cat' => 'Gestión', 'icon' => 'ph-list-dashes'],
                        ...
                        'schedule_manage' => ['label' => 'Administrar Eventos', 'cat' => 'Proyecto', 'icon' => 'ph-calendar-plus'],
                        'schedule_view_all' => ['label' => 'Ver Agenda Global', 'cat' => 'Proyecto', 'icon' => 'ph-eye'],
                        ...
                        'reporte_facturas' => ['label' => 'Rep. Facturas', 'cat' => 'Finanzas', 'icon' => 'ph-receipt'],
...
<style>
    ...
    .segment:has(input[value="1"]).active {
        color: var(--success);
    }

    .segment:has(input[value="0"]).active {
        color: var(--danger);
    }
...
<div id="deleteModal" class="modal-overlay">
    <div class="modal-container" style="text-align: center;">
        <div class="modal-header">
            <div
                style="width: 72px; height: 72px; background: rgba(239, 68, 68, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 1.5rem; animation: pulseRed 2s infinite;">
                <i class="ph-fill ph-trash" style="font-size: 36px; color: var(--danger);"></i>
            </div>
        </div>
        <div class="modal-body">
            <h3 style="font-size: 1.5rem; margin-bottom: 0.75rem; color: var(--text-main); font-weight: 600;">¿Eliminar
                Usuario?</h3>
            <p
                style="color: var(--text-muted); margin-bottom: 2.5rem; font-size: 0.95rem; line-height: 1.6; max-width: 80%; margin-left: auto; margin-right: auto;">
                Estás a punto de eliminar permanentemente a <strong id="deleteUserName"
                    style="color: var(--text-main);">UNKNOWN</strong>.<br>
